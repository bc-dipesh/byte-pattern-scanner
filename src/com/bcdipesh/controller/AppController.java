 ...
 * to the {@link AppView} using functions provided by the Class.
 *
 * @author Dipesh B.C.
 * @version 2.0
 */
public class AppController {
	...
	private int totalFilesInDir = 0;

	/**
	 * Creates a AppController Object that will link to the model, and the view
	 * passed to it.
	 *
	 * @param model Different Byte pattern matching operations will be done using
	 *              this object.
	...
	/**
	 * This class implements ActionListener and provides ActionListener object for
	...
	/**
	 * This class implements ActionListener and provides ActionListener object for
	 * the menu item load from a directory.
	 */
	...
	private class SearchPatternBtnListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!result.isEmpty() && !model.getIsDirectorySelected()) {
				...
				for (Map.Entry<Integer, byte[]> entry : result.entrySet()) {
					...
					resultTxt.append("Pattern found: ").append(pattern).append(", at offset: ").append(entry.getKey())
							.append(" (").append(hexValue).append(") within the file.\n");
					patternArr.clear();
				}
				...
				// ... Pre-formatting
				resultTxt.append("Directory: ").append(dirName).append("\t(").append(totalFilesInDir)
						.append(" files)\n");
				...
				for (Map.Entry<String, TreeMap<Integer, byte[]>> entries : foundPatterns.entrySet()) {
					...
					for (Map.Entry<Integer, byte[]> entry : entries.getValue().entrySet()) {
						...
						pattern = patternArr.toString().replaceAll("[,\\[\\]\\s+]", "");
						hexValue = String.format("0x%x", entry.getKey());
						resultTxt.append("Pattern found: ").append(pattern).append(", at offset: ")
								.append(entry.getKey()).append(" (").append(hexValue).append(") within the file.\n");
						patternArr.clear();
					}
				}
			} else if ((!model.getIsFileSelected()) && (!model.getIsDirectorySelected())) {
				resultTxt = new StringBuilder("Please load file/directory along with pattern file");
			} else {
	...
	/**
	 * Gets the file selected by the user. This function displays a file chooser GUI
